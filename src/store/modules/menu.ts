...
function addDynamicRoutes (menuList: RouteMenu[] = [], routes: AppRouteRecordRaw[] = []): AppRouteRecordRaw[] {
  let temp: RouteMenu[] = []
  for (let i = 0; i < menuList.length; i++) {
    const menu = menuList[i]
    if (menu?.children.length >= 1) temp = temp.concat(menu.children)
    if (menu.type === 'DIR' && menu?.children.length >= 1) menu.redirect = menu.children[0].path
...
export const useMenuStore = defineStore('menu', {
  ...
  actions: {
    SET_NAVTREE(navTree: RouteMenu[]){  // 设置导航菜单树
