...
export default defineComponent({
  ...
  setup(props, _ctx) {
    ...
    const renderLabel = ({ label, labelMinWidth, labelStyle }: DescItem) => {
      ...
      return <span class='text-gray-500' style={ labelStyles }>{label}{border ? '' : '：'}</span>
    }
    ...
    const renderContent = ({ render, field, contentMinWidth, contentStyle, slot }: DescItem) => {
      ...
      return <span class='break-all flex-1' style={contentStyles}>{ getContent() }</span>
    }
    ...
    const renderCell = (cellItem: DescItem, i: number, {component, type}: CellConfig) => {
      ...
      return <Component colspan={ calcSpan(i, type!) } key={`${type}-${i}`}>
