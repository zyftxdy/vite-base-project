...
export function useRender(
  ...
  const { handleCellMouseEnter, handleCellMouseLeave } = useEvents()
      ...
      createElementVNode(
        'div',
        {
          ...
          onMouseenter: ($event: MouseEvent) =>
            handleCellMouseEnter($event, rowData, wrapRef.value!),
  ...
  const dealColumns = (columns: TableColumn[]) => {
    ...
    columnsRef.value = columns.map(column => {
      if (!unref(isInit) && column.sortable) {
        sortState.value[column.prop] = TableV2SortOrder.ASC
      }
  ...
  const { setColumns, getColumns, getCacheColumns } = useColumns(props.columns, columns =>
    dealColumns(columns)
  )
  createTableContext({
    wrapRef,
  ...
  const calcWidth = () => {
    ...
    columns.map(column => {
      if (column.key !== SELECT_KEY) {
        column.width = Math.max(calcNum(averageWidth * column.widthScale), column.initWidth)
      }
    })
  }
  useWindowSizeFn({
    ...
    if (expanded) {
      ...
      const recursiveFn = (list: Recordable[]) => {
        ...
        list.map(item => {
          ...
          if (item.children) {
            flatKeys.push(...recursiveFn(item.children))
          }
        })
        return flatKeys
      }
