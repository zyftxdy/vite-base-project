  ...
  setup(props, { slots }) {
    ...
    return () => {
      ...
      const avatarDom = avatar ? (
        <div class='meta-avatar'>{ avatar() }</div>
      ) : null

      const titleDom = title ? (
        <div class={`meta-title ${textSize} ${fontWeight} padding-bottom5`} style={ titleStyle }>{ title }</div>
      ) : null

      const descriptionDom = description ? (
        <div class='meta-description text-sm text-gray-500' style={ descStyle }>{ description }</div>
      ) : null
