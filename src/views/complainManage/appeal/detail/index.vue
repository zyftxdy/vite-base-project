  ...
  <card class="complain_detail" title="申诉详情" :loading="loading">
    <div class="complain margin-bottom15">
      ...
      <description :schema="schema" :data="data">
        <template #schoolName>
          {{ data.schoolName
          }}<el-button link type="primary" class="ml-1" @click="goDetail(data.schoolId)"
            >查看</el-button
          >
        </template>
      </description>
        ...
        <el-form ref="formRef" :model="modelRef" :rules="rulesRef" label-width="100px">
          <el-form-item label="处理意见：" class="is-required">
            ...
            <div class="pl-4 text-red-600">
              支持家长，则申诉成功，本次课消记录将作废。支持机构，则申诉关闭，课消金额将划拨至校区可用账户
            </div>
          </el-form-item>
...
const {
  ...
  loading,
  run: getDetail
} = useRequest<Recordable>(id => reqApi.complain.appealDetail({ appealId: id }), {
  defaultValue: {},
  immediate: true,
