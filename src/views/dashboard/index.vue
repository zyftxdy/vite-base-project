<template>
  <div class="dashboard">
    ...
    <div class="content-box mb-4">
      <freeze-echart :dept-ids="deptIds" />
    </div>
    <div class="content-box mb-4">
      <freeze-list :dept-ids="deptIds" />
    </div>
    <div class="content-box mb-4">
      <thaw-echart :dept-ids="deptIds" />
    </div>
    <div class="content-box mb-4">
      <thaw-list :dept-ids="deptIds" />
    </div>
    <div class="content-box">
      <order-list :dept-ids="deptIds" />
    </div>
  </div>
</template>
...
const { result: dList } = useRequest<Recordable[]>(
  () => reqApi.dashboard.getSupeTotal({ deptIds: unref(deptIds) }),
  {
    defaultValue: dataList(),
    immediate: true,
    refreshDeps: [() => unref(deptIds)],
    formatResult: res => {
      const list: Recordable[] = unref(dList)
      return res
        ? list.map(item => {
...
const { result: tList } = useRequest<Recordable[]>(
  () => reqApi.dashboard.waitDoneTotal({ deptIds: unref(deptIds) }),
  {
    defaultValue: todoList(),
    immediate: true,
    refreshDeps: [() => unref(deptIds)],
    formatResult: res => {
      const list: Recordable[] = unref(tList)
      return res
        ? list.map(item => {
...
const goPage = (type: string) => {
  if (type === 'appeal' && appealPerm) {
    router.push('/appeal')
  }
  if (type === 'complaint' && complainPerm) {
