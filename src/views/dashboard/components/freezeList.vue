<template>
  <list-header title="资金监管统计" desc="仅统计已经成功创建订单的支付流水" @dateChange="dateChange"/>
  <usual-table
    :loading="loading"
    ...
    @pagination="getList">
    <template #action="{ row }">
      <el-button v-auth:disabled="'09444fd98fc1f87508'" link type="primary" @click="goDetail(row.payTime)">查看详情</el-button>
    </template>
  </usual-table>
</template>
...
const { loading, run: getList } = useRequest(() => reqApi.dashboard.freezeList({
  ...queryState,
