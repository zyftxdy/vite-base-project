<template>
  ...
  <usual-table :loading="loading" :columns="columns" :list="list">
    <template #action="{ row }">
      <el-button
        v-auth:disabled="'09444fd98fc1f87003'"
        link
        type="primary"
        @click="goDetail(row.schoolId, row.orderId)"
        >查看订单</el-button
      >
    </template>
  </usual-table>
...
const {
  ...
  loading,
  run: getList
} = useRequest<Recordable[]>(() => reqApi.dashboard.getSupeList({ deptIds: props.deptIds }), {
  defaultValue: [],
  immediate: true,
