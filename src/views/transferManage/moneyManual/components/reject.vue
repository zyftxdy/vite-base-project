<template>
  ...
  <description v-if="info.transferStatus === 'APPROVE_UNPASS'" :schema="rejectSchema" :data="info"/>
  <el-form
    v-if="showOper"
    ref="formRef"
    :model="modelRef"
...
const { result: info, run: getDetail } = useRequest<Recordable>(id => reqApi.transfer.manualInfo({ transferId: id }), {
  defaultValue: {}
})
watch(() => props.id, id => id && getDetail(id),  { immediate: true })
...
const { run: submit } = useRequest(() => reqApi.transfer.manualApply({
  transferId: props.id,
  ...modelRef
}), {
