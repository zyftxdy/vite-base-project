...
const { loading, run: getList } = useRequest(() => reqApi.transfer.rulePage(queryState.listQuery), {
  immediate: true,
  onSuccess: res => {
...
const handleDel = (id: string) => {
  messageConfirm({ message: '确定要删除该配置吗？' })
    .then(() => {
      del(id)
    })
    .catch()
}
const { run: del } = useRequest(id => reqApi.transfer.ruleDel({ ruleId: id }), {
  onSuccess: () => {
