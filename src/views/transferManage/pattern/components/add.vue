  ...
  <el-form
    ...
    <el-form-item v-if="modelRef.transferType" label="规则内容：" class="is-required">
      <div
        ...
        <span class="pl-1">划拨比例：<el-input v-model="item.scale" placeholder="划拨比例" class="w-32"><template #append>%</template></el-input></span>
        <span class="cursor-pointer text-xl pl-2">
          <template v-if="index === modelRef.scaleRuleDetails.length - 1 && index < 12"><el-icon class="align-middle" @click="add"><i-ep-circlePlus /></el-icon></template>
          <template v-if="index !== 0"><el-icon class="align-middle" @click="del"><i-ep-circleClose /></el-icon></template>
        </span>
      </div>
      <template v-else>
        <div class="apply-f-m my-1">
          单次申请资金超过<el-input v-model="modelRef.singleLimit" class="w-16 mx-1"/>元，转为人工审批；
        </div>
        <div class="apply-f-m my-1">
          累计<el-input v-model="modelRef.limitDay" class="w-16 mx-1"/>天，划拨超过<el-input v-model="modelRef.totalLimit" class="w-16 mx-1"/>元，转为人工审批；
        </div>
        <div class="apply-f-m my-1">
          划拨后保证金账户低于<el-input v-model="modelRef.miniLimit" class="w-16 mx-1"/>元，转为人工审批。
        </div>
...
emitter.on('submit', () => {
  ...
  if ((modelRef.transferType === 'SCALE' && result) ||
    (modelRef.transferType === 'APPLY' && (
      !modelRef.singleLimit ||
      !modelRef.limitDay ||
      !modelRef.totalLimit ||
      !modelRef.miniLimit
    ))
