...
export default defineComponent({
  ...
  setup(props, _ref) {
    ...
    const { setOptions: setOptionL, getInstance: getInstanceL } = useEcharts(
      elRefL as Ref<HTMLDivElement>
    )
    const { setOptions: setOptionR, getInstance: getInstanceR } = useEcharts(
      elRefR as Ref<HTMLDivElement>
    )

    const { result, run: getData } = useRequest<Recordable>(
      () => reqApi.statistics.statisCredit({ deptIds: props.deptIds }),
      {
        defaultValue: {
          white: 0,
          grey: 0,
      ...
      return (
        <div class="center_bottom">
          ...
          <div class="c_t">
            ...
            <div class="c_content">
              <div class="c_c_c">
                <count-to value={unref(result).white} styles={styles} />
                <span class="c_t">白名单</span>
              </div>
              <div class="c_c_c">
                <count-to value={unref(result).grey} styles={styles} />
                <span class="c_t">灰名单</span>
              </div>
              <div class="c_c_c">
                <count-to value={unref(result).black} styles={styles} />
