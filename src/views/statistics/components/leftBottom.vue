<template>
  ...
  <div class="left_bottom">
    <div class="l_b_t margin-bottom30">
      ...
      <count-to :value="result.unfreezeAmountTotal" :to-fixed="2" :styles="styles" />
    </div>
    <div class="l_b_i">
      <div class="l_b_c">
        <div class="t">解冻总课时数</div>
        <count-to :value="result.unfreezeClassHourTotal || 0" :styles="styles" />
      </div>
      <div class="l_b_c">
        <div class="t">解冻失败金额（元）</div>
        <count-to :value="result.unfreezeFail" :to-fixed="2" :styles="styles" />
      </div>
    </div>
  </div>
</template>
...
const { result } = useRequest<Recordable>(
  () =>
    reqApi.statistics.statisFree({
