  ...
  <el-skeleton :rows="5" :loading="loading" animated>
    <div class="transfer_hand">
      ...
      <div class="pb-3 text-base font-medium">
        <animation-text
          source="您正在手动操作资金划拨，手动划拨完成后监管账户资金将立刻转入教培机构一般账户内。因本次手动操作记录无法与系统原一课一消数据关联，有可能导致监管系统中财务数据出现对账错误等问题，请您注意并确认您有权执行本次操作！如有疑问请联系：400-829-1566。"
        />
      </div>
      ...
      <div class="submit_form">
        <el-form ref="formRef" :model="operState" :rules="rulesRef">
          <el-form-item label="划拨金额：" prop="trdAmt">
            <el-input
              v-model="operState.trdAmt"
              :disabled="!!operState.transferSummaryId"
              placeholder="金额需小于当前监管金额"
            />
          </el-form-item>
        ...
        <div class="text-center">
          <el-button class="btn-small btn-border-color" type="primary" @click="handleSubmit"
            >确认提交</el-button
          >
          <el-button v-if="isEmpty(params)" class="btn-small btn-usual" @click="$emit('back')"
            >返回列表</el-button
          >
        </div>
      </div>
      <el-dialog v-model="operState.show" title="提示" width="880px" :close-on-click-modal="false">
...
const {
  ...
  loading,
  run: getDetail
} = useRequest<Recordable>(
  data =>
    operState.transferSummaryId
      ? reqApi.transfer.transInfo(data)
...
const { loading: operLoading, run: handleOk } = useRequest(
  () =>
    reqApi.school.transferHand({
