<template>
  <el-skeleton :rows="5" :loading="loading" animated>
        ...
        <div class="info">
          ...
          <description v-if="showOper" :schema="bankSchema" :data="schoolInfo">
            <template #bankEarnestMoney>
              <span
                ><span class="text-2xl text-red-600 pr-1">{{
                  schoolInfo.bankEarnestMoney ?? '--'
                }}</span
                >元</span
              >
              <el-button
                v-auth:hide="'09444fd98fc1f87004'"
                link
                type="primary"
                class="ml-1"
    ...
    <usual-dialog v-model:show="operState.show">
      <div class="pb-2 text-base font-medium">
        注意：仅当风险保证金存缴方式无法通过系统自动获取时需要手动在此设置，请您注意并确认您有权执行本操作！
      </div>
...
const schoolId = ref('')
const { result: schoolInfo, loading } = useRequest<Recordable>(
  () => reqApi.school.schoolInfo({ schoolId: unref(schoolId) }),
  {
    defaultValue: {},
    immediate: true,
    ...
    onSuccess: res => {
      ...
      subjectInfo.licenseHasSchoolPermit &&
        subjectInfo.qual_list.push({ name: '办学许可证', src: subjectInfo.licenseSchoolPermit })
      subjectInfo.qual_list.push({ name: '银行开户凭证', src: subjectInfo.licenseAccountPhoto })
      subjectInfo.qual_list.push({
        name: '法人身份证正面',
        src: subjectInfo.legalPersonIdcardPhotoA
      })
      subjectInfo.qual_list.push({
        name: '法人身份证反面',
        src: subjectInfo.legalPersonIdcardPhotoB
      })
...
const { run: changeEarnestMoney } = useRequest(
  () =>
    reqApi.school.handMoney({
